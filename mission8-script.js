...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const mission8Reflection = document.getElementById('mission8-reflection');

    btnSubmitReflection?.addEventListener('click', () => {
        const reflection = mission8Reflection.value.trim();
        ...
        if (window.TaskProgress) {
            const completed = window.TaskProgress.completeTask('mission8');
            if (completed) {
                window.TaskProgress.showTaskCompleteNotification('mission8');
                if (window.EsgStats) {
                    window.EsgStats.recordMissionCompletion('mission8', {
                        notesAdded: 1,
                        askRating: true
                    });
                }
                if (window.TravelerStore) {
                    window.TravelerStore.recordMissionCompleted('mission8', {
                        notesAdded: 1
                    });
                }
            }
